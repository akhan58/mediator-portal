...
async function syncExternalReviews(userId) {
    try {
        ...
        await syncPlatformReviews('trustpilot', business.trustpilot_businessunit_id, userId);
...
async function syncPlatformReviews(platform, platformId, userId) {
    ...
    const existingReviews = await reviewsAccessLayer.getReviewByPlatform(platform);
    ...
    for (const existingReview of existingReviews) {
        ...
        if (currentReview.content !== existingReview.content || 
