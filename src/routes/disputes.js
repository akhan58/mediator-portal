...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      const dispute = await disputesAccessLayer.getDisputeById(
...
router.get("/reviewId/:reviewId", validateInt("reviewId"), async (req, res) => {
  ...
  try {
    const dispute = await disputesAccessLayer.getDisputeByReviewId(
      req.params.reviewId,
    );
    if (!dispute) {
      return res.status(404).json({ error: "Dispute not found" });
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      const dispute = await disputesAccessLayer.getDisputesByStatus(
        req.params.disputeStatus,
      );
      if (!dispute) {
        return res.status(404).json({ error: "Dispute not found" });
      }
...
router.delete("/:disputeId", validateInt("disputeId"), async (req, res) => {
  ...
  try {
    const dispute = await disputesAccessLayer.deleteDispute(
      req.params.disputeId,
    );
