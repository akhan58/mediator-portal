  ...
  try {
    ...
    for (const review of reviews) {
      ...
      const existingReview =
        await reviewsAccessLayer.getReviewsByPlatformAndSourceId(
          review.platform,
          review.source_id,
        );
      ...
      if (existingReview.length === 0) {
        const storedReview = await reviewsAccessLayer.createReview(
          usersIdToReviews,
        );
...
router.get("/trustpilot", auth, async (req, res) => {
  try {
    ...
    if (!business.trustpilot_businessunit_id) {
      return res
        .status(400)
        .json({ error: "Trustpilot Business Unit ID not configured" });
    }

    // Fetch reviews from Trustpilot API
    const { reviews, error } = await fetchTrustpilotReviews(
...
router.get("/yelp", auth, async (req, res) => {
  try {
    ...
    const { reviews, error } = await fetchYelpReviews(
...
router.get("/facebook", auth, async (req, res) => {
  try {
    ...
    const { reviews, error } = await fetchFacebookReviews(
...
router.get("/", auth, async (req, res) => {
  try {
    ...
    const reviews = await reviewsAccessLayer.getReviewByUserId(req.user_id);
...
router.post("/sync", auth, async (req, res) => {
  try {
    const result = await syncExternalReviews(req.user_id);
