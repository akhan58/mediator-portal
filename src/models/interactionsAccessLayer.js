...
const interactionsAccessLayer = {
  ...
  async createInteraction({ reviewId, responseText, userId }) {
    const results = await pool.query(
      `INSERT INTO interactions (review_id, response_text, user_id) VALUES ($1, $2, $3) RETURNING *`,
  ...
  async getInteractionById(responseId) {
    const results = await pool.query(
      `SELECT * FROM interactions WHERE "response_id" = $1`,
      [responseId],
    );
    return results.rows;
  },
  ...
  async getInteractionByReviewId(reviewId) {
    const results = await pool.query(
      `SELECT * FROM interactions WHERE "review_id" = $1`,
      [reviewId],
    );
    return results.rows;
  },
  ...
  async updateResponseText({ responseId, responseText }) {
    const results = await pool.query(
      `UPDATE interactions SET response_text=$2 WHERE "response_id"=$1 RETURNING *`,
  ...
  async deleteInteraction(responseId) {
    const results = await pool.query(
      `DELETE FROM interactions WHERE "response_id" = $1`,
      [responseId],
    );
    return results.rows;
  },
